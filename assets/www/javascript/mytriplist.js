function initMyTrip(){
    ...
    var mytriplist= $.parseJSON(storage.getItem('detail'));
//    debugger;
    var mytripcontent="";
    for(var i=0;i<mytriplist.length;i++){
        mytripcontent+='<li><a href="#" id="'+mytriplist[i].id+'">'+
            '<img src="http://img03.taobaocdn.com/tps/i3/T1jJ3UXaleXXceHVsE-115-115.jpg" />'+
            '<h3>'+mytriplist[i].name+'</h3>'+
            '<p>'+mytriplist[i].start_time+'</p>'+
        '<p><span class="routeto">'+mytriplist[i].position+'</span><span class="photos">'+mytriplist[i].image+'</span><span class="records">'+mytriplist[i].voice+'</span></p>'+
        '</a></li>';
    }
    $('.mytrip_ul').append(mytripcontent);
    $("#mytrip").page();
    $(".mytrip_ul").listview('refresh');

    $(document).ready(function(){
        $(".mytrip_ul a").click(function(e){
            $(document).data("id",$(e.currentTarget).attr("id") );
            $.mobile.changePage("#tour-detail");
        });
    });
